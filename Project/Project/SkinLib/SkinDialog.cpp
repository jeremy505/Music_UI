...
//#define __DrawEclosionText
#ifdef __DrawEclosionText
// 绘制羽化文字
...
CSkinDialog::CSkinDialog(void)/*:m_hWndRgn(NULL)*/
{
	...
	m_rcLeftPan = CRect(0, 0, 0, 0);
	m_rcPlayPan = CRect(0, 0, 0, 0);
	...
	//左菜单
	m_lpLeftPanBfIm = NULL;
	//播放菜单
	m_lpPlayPanBfIm = NULL;
...
BOOL CSkinDialog::SetLeftPanPic(LPCTSTR lpFileName)
{
	CSkinManager::GetInstance()->ReleaseImage(m_lpLeftPanBfIm);

	m_lpLeftPanBfIm = CSkinManager::GetInstance()->GetImage(lpFileName);

	return (NULL == m_lpLeftPanBfIm) ? FALSE : TRUE;
}

BOOL CSkinDialog::SetPlayPanPic(LPCTSTR lpFileName)
{
	CSkinManager::GetInstance()->ReleaseImage(m_lpPlayPanBfIm);

	m_lpPlayPanBfIm = CSkinManager::GetInstance()->GetImage(lpFileName);

	return (NULL == m_lpPlayPanBfIm) ? FALSE : TRUE;
}
...
void CSkinDialog::OnPaint(CDCHandle dc)
{
	...
	if (m_lpLeftPanBfIm != NULL && !m_lpLeftPanBfIm->IsNull())// 左菜单背景图片
	{
		//m_lpLeftPanBfIm->Draw2(MemDC.m_hDC, m_rcLeftPan);
		m_lpLeftPanBfIm->Draw(MemDC.m_hDC, m_rcLeftPan);
	}

	if (m_lpPlayPanBfIm != NULL && !m_lpPlayPanBfIm->IsNull())
	{
		m_lpPlayPanBfIm->Draw(MemDC.m_hDC, m_rcPlayPan);
	}
...
// 	::DeleteObject(hPen);
	...
	if (nTitleTextLen > 0)
	{
		...
		int nNumerator = ::GetDeviceCaps(MemDC.m_hDC, LOGPIXELSY);
		nif.lfSmCaptionFont.lfHeight = ::MulDiv(6*2, -nNumerator, 72);
	//	nif.lfSmCaptionFont.lfWeight = 0;
		...
		//jeremy add to change the word


		//LOGFONT lFont;
		//计算字体高度与磅值 
		//int nNumerator = ::GetDeviceCaps(MemDC.m_hDC, LOGPIXELSY);
		//lFont.lfHeight = ::MulDiv(2*2, -nNumerator, 72);
		//lFont.lfWeight = 0*200;
		//绘制 
		/**
		...
		::DeleteObject(hFont);
	**/
		::DrawText(MemDC.m_hDC, m_strTitleText, nTitleTextLen, &rcText, DT_SINGLELINE | DT_LEFT | DT_VCENTER);
		//释放GDI资源 
		::SelectObject(MemDC.m_hDC, hOldFont);
		::SetBkMode(MemDC.m_hDC, nMode);
...
void CSkinDialog::CalcTitleBarRect()
{
	...
	m_rcTitleBar = CRect(0, 0, rcClient.Width(),50);


	m_rcLeftPan = CRect(0, 50, 200, rcClient.Height() - 50);
	m_rcPlayPan = CRect(0, rcClient.Height() - 50, rcClient.Width(), rcClient.Height());
	if (m_bHasCloseBtn)
	{
		m_rcCloseBtn = CRect(rcClient.Width() - 30 - 10, 12, rcClient.Width() - 10, 38);

		if (m_bHasMaxBtn)
		{
			m_rcMaxBtn = CRect(rcClient.Width() - 10 - 30 - 30, 12, rcClient.Width() - 10 - 30, 38);
		}
	}
	...
	if (m_bHasMinBtn)
	{
		if (!m_bHasMaxBtn)
		{
			m_rcMinBtn = CRect(rcClient.Width() - 10 - 30 - 30, 12, rcClient.Width() - 10 - 30, 38);
		}
		else
		{
			m_rcMinBtn = CRect(rcClient.Width() - 10 - 30 - 30 - 30, 12, rcClient.Width() - 10 - 30 - 30, 38);
		}
	}
}
